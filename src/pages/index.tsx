...
function ImageBrowser({
  ...
  imagesData: { images: typeof images };
}) {
    ...
    >
      ...
      <Lightbox
        ...
        slides={imagesData.images.map((image) => {
          return {
            src: image.gatsbyImageOriginalUrl
              ? image.gatsbyImageOriginalUrl
              : image.url,
            alt: image.alt,
            title: image.title,
    ...
    <Box display="flex" flexDirection="column">
      ...
      >
        <Text>
          ...
          <Link href={aboutData.about.wikiUrl}>(Wikipédia)</Link>
        </Text>
...
function StudentGroups({
  ...
  studentGroupsData: { groups: Array<StudentGroupType> };
}) {
  return (
...
function Presidency({
  ...
  presidencyData: { profiles: Array<ProfileType> };
}) {
  return (
      ...
      return {
        ...
        profilePicture: {
          ...
          gatsbyImageData: data.simonyi_elnokok.edges.find((edge) => {
            return (
              `images/${edge.node.relativePath}` === profile.profilePicture.url
            );
          })?.node.childImageSharp?.gatsbyImageData,
        },
      };
...
export const query = graphql`
  query homePage {
    simonyi_elnokok: allFile(
      filter: {
        sourceInstanceName: { eq: "images" }
        relativeDirectory: { eq: "simonyi/elnokok" }
      }
    ) {
      edges {
        node {
